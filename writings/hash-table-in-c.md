...
You can find the code for this implementation in the [hyblancode/ht](https://github.com/hyblancode/ht) repo on GitHub, in [ht.h](https://github.com/hyblancode/ht/blob/master/ht.h) and [ht.c](https://github.com/hyblancode/ht/blob/master/ht.c). For what it's worth, all the code is released under a permissive MIT license.

I got some [good feedback](https://codereview.stackexchange.com/questions/257634/hash-table-implemented-in-c-with-open-addressing/257649) from Code Review Stack Exchange that helped clean up a few sharp edges, not the least of which was a memory leak due to how I was calling `strdup` during the `ht_expand` step (fixed [here](https://github.com/hyblancode/ht/commit/970ba8ca3ddef5d2aa1d7a36da290f380a87115f)). I confirmed the leak [using Valgrind](https://stackoverflow.com/questions/5134891/how-do-i-use-valgrind-to-find-memory-leaks), which I should have run earlier. Seth Arnold also gave me some helpful feedback on a draft of this article. Thanks, folks!
...
After a couple of iterations, I settled on the following functions and structs (see [ht.h](https://github.com/hyblancode/ht/blob/master/ht.h)):
...
Now let's turn to the hash table implementation ([ht.c](https://github.com/hyblancode/ht/blob/master/ht.c)).
...
That's it -- the implementation in [ht.c](https://github.com/hyblancode/ht/blob/master/ht.c) is only about 200 lines of code, including blank lines and comments.
...
As I mentioned, I wanted to keep the implementation simple, and wasn't too worried about performance. However, a quick, non-scientific [performance comparison](https://github.com/hyblancode/ht/blob/master/samples/perftest.sh) with Go's `map` implementation shows that it compares pretty well -- with half a million English words, this C version is about 50% slower for [lookups](https://github.com/hyblancode/ht/blob/master/samples/perfget.c) and 40% faster for [insertion](https://github.com/hyblancode/ht/blob/master/samples/perfset.c).

Speaking of Go, it's even easier to write custom hash tables in a language like Go, because you don't have to worry about handling memory allocation errors or freeing allocated memory. I recently wrote a [counter](https://github.com/hyblancode/counter) package in Go which implements a similar kind of hash table.
