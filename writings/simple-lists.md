...
```
$ git clone https://github.com/hyblancode/simplelists.git
...
See the [source code](https://github.com/hyblancode/simplelists), which we'll describe further below. Let's dive in!
...
I still don't love the verbosity of Go's error handling, particularly in HTTP request handlers, where the `return` is on its own line. One line of code, four lines of error handling. It doesn't make the code difficult to understand, just adds a bit of noise. For example, compare [this handler code in server.go](https://github.com/hyblancode/simplelists/blob/228b28e8d418604f7176d1980028f8bd139e537a/server.go#L230-L238):
...
Here are a couple of functions from [`db.go`](https://github.com/hyblancode/simplelists/blob/master/db.go) to give you the flavour of what this looks like. `GetList` fetches a single list, and the `getListItems` helper fetches all of that list's items:
...
See the [full template source](https://github.com/hyblancode/simplelists/blob/master/templates.go). Note the `meta` viewport tag, which makes the layout work nicely on mobile phones -- Simple Lists is "fully responsive"!
