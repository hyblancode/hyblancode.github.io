...
*Update July 2020: it's actually not O(log N), but a somewhat complicated power law that's between log N and N, depending on the distance threshold. See Maximilian Knespel's [detailed analysis](https://github.com/hyblancode/pybktree/issues/5).*
...
So again, our Python code is available [on GitHub](https://github.com/hyblancode/pybktree) and [from the Python Package Index](https://pypi.python.org/pypi/pybktree) -- so it's only a `pip install pybktree` away.
